class UIManager:
     ...
     Atributos:
         root: Janela principal da interface Tkinter.
         api_manager: Gerenciador de API para comunicação com LLM.
         screen_capture: Módulo de captura de tela.
         privacy_manager: Gerenciador de privacidade.
         response_overlay: Janela de sobreposição de resposta.
         preview_window: Janela de pré-visualização.
         hidden_mode: Estado do modo oculto.
     """
     def __init__(self):
         """
         Inicializa o gerenciador de interface do usuário e seus componentes.
         """
         self.root = tk.Tk()
     ...
     def _build_ui(self):
         """
         Constrói a interface do usuário com botões e campos de entrada.
         """
     ...
     def _send_to_llm(self):
         """
         Envia o texto inserido para a API do LLM e exibe a resposta.
         """
     ...
     def _async_send(self, texto):
         """
         Envia a pergunta de forma assíncrona e exibe a resposta ou erro.
 
         Parâmetros:
             texto (str): Texto da questão a ser enviado.
         """
         try:
     ...
     def show_response_overlay(self, resposta):
         """
         Exibe a resposta do LLM em uma janela de sobreposição.
 
         Parâmetros:
             resposta (str): Resposta recebida da API.
         """
     ...
     def set_status(self, msg):
         """
         Atualiza a barra de status com a mensagem fornecida.
 
         Parâmetros:
             msg (str): Mensagem a ser exibida na barra de status.
         """
         self.status_var.set(msg)
 
     def toggle_privacy_mode(self):
         """
         Alterna o modo oculto, ocultando ou mostrando janelas sensíveis.
         """
     ...
     def _restore_privacy_visual(self):
         """
         Restaura o visual original após o teste do modo oculto.
         """
         if not self.hidden_mode:
             self.privacy_manager.show_sensitive_windows()
             self.toggle_privacy_btn.config(text="Ativar Modo Oculto")
